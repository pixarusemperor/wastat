...
1. **Read the env contract first** (`.env.example`, server env reads). A required var missing in prod = 100% of requests return 500 while the container shows "healthy" — TCP healthchecks cannot see app-level validation failures.
